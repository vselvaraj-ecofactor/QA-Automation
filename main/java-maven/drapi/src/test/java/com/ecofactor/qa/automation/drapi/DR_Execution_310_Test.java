...
public class DR_Execution_310_Test {
	...
	public void drEventForEco(final String drUrl, final String programID,
			final String eventID, final String targetType,
			final String targetALLJson, final String tstId)
			throws ParseException {
		...
		json = json.replaceFirst(
				"<start_time>",
				Long.toString(DateUtil
						.addToUTCMilliSeconds(Calendar.MINUTE, 40)))
				.replaceFirst(
						"<end_time>",
						Long.toString(DateUtil.addToUTCMilliSeconds(
								Calendar.MINUTE, 65)));
		setLogString("URL Values of the API \n" + createUrl + "\n" + json, true);

		final HttpResponse response = HTTPSClient.postResponse(createUrl, json,
				HTTPSClient.getPKCSKeyHttpClient("ecofactorcorp.p12",
						"ecofactor"));
		...
		final Map<String, Object> values = lsProEvent
				.updateEventByStartDateAndEndDate(eventName);
		...
		final List<Integer> eventLocation = lspEventLocation
				.fetchByLocationId(proramEventId);
		...
		final Map<String, Object> startTime = lsProEvent
				.updateEventByStartDate(eventName);
		setLogString("Details after updated the start time : " + startTime,
				true);

		WaitUtil.hugeWait();
		setLogString("After 5 mins ", true);

		// Now event should have status as ACTIVE. we need an assert statement
		// here
		final String status = lsProEvent.listByEventName(eventName);
		setLogString("DR Event Status after 5 mins : " + status, true);
		Assert.assertTrue(status.equalsIgnoreCase(ASTATUS),
				"Still status is not in ACTIVE mode");

		// This should have location Ids with status as SKIPPED and ACTIVE

		final List<String> locIdStatus = lspEventLocation
				.fullListByLocationIdStatus(proramEventId);
		...
		setLogString("Location and Thermostat Count based on status : "
				+ countDetails, true);
		...
		// event
		/*
		 * Action Item: {start event enty:Assertions on algorithm 210 should
		 ...
		 * action endEvent, groupevent_id = event_id.2(2713.2)}, {pre cool
		 * entry: assert there are three -50 algo control entry 1.) event phase
		 * 0 and new_setting as 80.00 and event_type LS 2.) event phase 1 and
		 * new_setting as 84.00 and event_type LS 3.) event phase 2 and
		 * new_setting as 80.00 and event_type LS} As an eg: on qa-plat run this
		 * query : select * from ef_thermostat_event where thermostat_id = 2641
		 * and group_event_id in (2715,2715.2)
		 */

		/*
		 * List<PartitionedThermostatEvent> allDetails =
		 * tstatEvent.std210DR(thermostatId, grpId);
		 * setLogString(allDetails.toString(), true);
		 */

		final List<ThermostatEvent> tstatDetails = efTsatEvent
				.fetchDetailsByThermostatId(thermostatId, grpId);
		...
		Assert.assertTrue(isEntryValues(tstatDetails, algorithmId),
		...
		final List<ThermostatEvent> tstatDetails1 = efTsatEvent
				.fetchDetailsByThermostatId(thermostatId, grpEventIdCompleted);
		System.out.println(tstatDetails1);
		isEndEntryValidated(tstatDetails1, grpEventIdCompleted);
		/*
		 * Assert.assertTrue(isEndEntryValidated(tstatDetails1,
		 * grpEventIdCompleted), "Values were not matched in End Entry");
		 */
		 ...
		 * lspEventReport.updatedDetails(proramEventId);
	...
	private String getDrEventName(final String response) {
		...
		setLogString(
				"DR EventName Fetched : "
						+ eventName.substring(0, eventName.length() - 3), true);
	 ...
	 * @return Integer.
	 */
	public int getLocationIdOfStatus(Map<Object, Object> allDetails,
			String status) {
	 ...
	 */
	public List<Thermostat> getCountOfThermostatBasedOnStatus(
			final Integer programEventId,
		...
		for (final EcpCoreLSEventLocation locationEvent : locationByStatus) {
			...
			list.addAll(tsat.listThermostatsByLocation(locationEvent
					.getLocationid()));
		}
		return list;
	}
	 ...
	 */
	public Map<String, Integer> getCountOfTstLocations(
			final Integer proramEventId) {
		...
		tstByActiveStatus = getCountOfThermostatBasedOnStatus(proramEventId,
				locationByStatus);
		...
		tstBySkippedStatus = getCountOfThermostatBasedOnStatus(proramEventId,
				locationByStatus1);
	 ...
	 */
	public boolean isLocationCountValidated(
			final Map<String, Integer> locationCount,
			final Map<String, Object> detail) {
		...
		for (Map.Entry<String, Integer> values : locationCount.entrySet()) {
			for (Map.Entry<String, Object> value : detail.entrySet()) {
				...
				if ((values.getKey()
						.contains("Total No of Locations based on Active Status"))
	...
	public boolean isTstCountValidated(final Map<String, Integer> tstCount,
		...
		for (Map.Entry<String, Integer> values : tstCount.entrySet()) {
			for (Map.Entry<String, Object> value : detail.entrySet()) {
				...
				if ((values.getKey()
						.contains("Total No of Thermostats based on Active Status"))
	...
	public boolean isStartEntryValidated(final List<ThermostatEvent> details) {
		...
		for (ThermostatEvent thermostatEvent : details) {
			if ((thermostatEvent.getAction().equalsIgnoreCase(STARTEVENT))
					...
					&& (thermostatEvent.getEventStatus()
							.equalsIgnoreCase(EVENTSTATUS))
					&& (thermostatEvent.getEventType()
							.equalsIgnoreCase(EVENTYPE_ALGO))) {
	...
	public boolean isEndEntryValidated(final List<ThermostatEvent> details,
		...
		for (ThermostatEvent thermostatEvent : details) {
			...
			System.out.println("ALGORITHMID_210"
					+ thermostatEvent.getAlgorithmId());
			System.out
					.println("EVENTPHASE_2" + thermostatEvent.getEventPhase());
			System.out
					.println("EVENTSTATUS" + thermostatEvent.getEventStatus());
			System.out
					.println("EVENTYPE_ALGO" + thermostatEvent.getEventType());
			System.out.println("grpEventIdCompleted"
					+ thermostatEvent.getGroupEventId());
			if ((thermostatEvent.getAction().equalsIgnoreCase(ENDEVENT))
					...
					&& (thermostatEvent.getEventType()
							.equalsIgnoreCase(EVENTYPE_ALGO))
					&& (grpEventIdCompleted.equals(thermostatEvent
							.getGroupEventId()))) {
	 ...
	 */
	public boolean isEntryValues(final List<ThermostatEvent> details,
			final Integer algorithmId) {
		...
		for (ThermostatEvent thermostatEvent : details) {
			if (algorithmId.equals(ALGORITHMID_210)) {
				if (thermostatEvent.getAlgorithmId().equals(ALGOID_MINUS_FIFTY)) {
					...
					final int newSettingValue = thermostatEvent.getNewSetting()
							.intValue();
					eventPhase0 = checkEventPhase0(thermostatEvent,
							eventPhase0, newSettingValue);
					eventPhase1 = checkEventPhase1(thermostatEvent,
							eventPhase1, newSettingValue);
					eventPhase2 = checkEventPhase2(thermostatEvent,
							eventPhase2, newSettingValue);
					validate = true;
					break;
				}
	 ...
	 * Check event phase0.
	 * 
	 * @param thermostatEvent
	 *            the thermostat event
	 * @param eventPhase0
	 *            the event phase0
	 * @param newSettingValue
	 *            the old setting value
	...
	private boolean checkEventPhase0(final ThermostatEvent thermostatEvent,
		...
		return eventPhase0
				|| (thermostatEvent.getEventPhase().equals(EVENTPHASE_0)
						&& thermostatEvent.getEventType().equalsIgnoreCase(
								EVENTYPE_LS) && thermostatEvent.getNewSetting()
						.equals(newSettingValue));
	}
