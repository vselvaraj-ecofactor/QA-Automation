...
public class CancelDRAPI_Test extends AbstractTest {
	...
	public void drCancelEventForNVE(final String drUrl, final String programID,
			final String eventID, final String targetType,
			final String targetALLJson, final String cancel_url,
			final String cancel_dr_json) throws JSONException {
		...
		cancelURL = cancelURL.replaceFirst("<program_id>", programID)
				.replaceFirst("<PARTNER_EVENT_ID>", eventId);
		System.out.println(cancelURL);
		String cancel_json = cancel_dr_json;
		System.out.println(cancel_json);

		final HttpResponse cancel_response = HTTPSClient.putResponse(cancelURL,
				cancel_json, HTTPSClient.getPKCSKeyHttpClient(
						"ecofactorqanve.p12", "ecofactor"));
		Assert.assertTrue(
				cancel_response.getStatusLine().getStatusCode() == 200,
				"Error Status:" + cancel_response.getStatusLine());

	}

	@Test(groups = { Groups.SANITY1 }, dataProvider = "cancelDREventComCAST", dataProviderClass = DRAPIDataProvider.class, priority = 1)
	public void drCancelEventForComcast(final String drUrl,
			final String programID, final String eventID,
			final String targetType, final String targetALLJson,
			final String cancel_url, final String cancel_dr_json)
	...
	/*
	 * @Test(groups = { Groups.SANITY1 }, dataProvider = "cancelDREventECO",
	 * dataProviderClass = DRAPIDataProvider.class, priority = 1) public void
	 * drCancelEventForECO(final String drUrl, final String programID, final
	 * String eventID, final String targetType, final String targetALLJson,
	 * final String cancel_url,final String cancel_dr_json) throws JSONException
	 * {
	 * 
	 * long timeStamp = System.currentTimeMillis(); String createUrl = drUrl;
	 * 
	 * createUrl = createUrl.replaceFirst("<program_id>", programID)
	 * .replaceFirst("<event_id>", eventID + timeStamp)
	 * .replaceFirst("<target_type>", targetType).replaceFirst("<target_all>",
	 * "true");
	 * 
	 * String json = commonJson(createUrl, targetALLJson);
	 * 
	...
	public void drCancelEventForDefault(final String drUrl,
			final String programID, final String targetType,
			final String targetALLJson, final String cancel_url,
			final String cancel_dr_json) throws JSONException {
		...
		final HttpResponse response = HTTPSClient.postResponse(createUrl, json,
				HTTPSClient.getPKCSKeyHttpClient("nopartnercode.p12",
						"ecofactor"));
	...
	public void drCancelAlreadyCancelledEvent(final String programID,
			final String event_id, final String cancel_url,
			final String cancel_dr_json) throws JSONException {
		String cancelURL = cancel_url;
		...
		final HttpResponse cancel_response = HTTPSClient.putResponse(cancelURL,
				cancel_json, HTTPSClient.getPKCSKeyHttpClient(
						"nopartnercode.p12", "ecofactor"));
		Assert.assertTrue(
				cancel_response.getStatusLine().getStatusCode() == 400,
				"Error Status:" + cancel_response.getStatusLine());
		final String resultValueString = HTTPSClient
				.getResultString(cancel_response.getEntity());
	...
	public void drCancelNonExistingEvent(final String programID,
			final String event_id, final String cancel_url,
			final String cancel_dr_json) throws JSONException {
		String cancelURL = cancel_url;
		String eventId = event_id;
		...
		final HttpResponse cancel_response = HTTPSClient.putResponse(cancelURL,
				cancel_json, HTTPSClient.getPKCSKeyHttpClient(
						"nopartnercode.p12", "ecofactor"));
		Assert.assertTrue(
				cancel_response.getStatusLine().getStatusCode() == 404,
				"Error Status:" + cancel_response.getStatusLine());
		final String resultValueString = HTTPSClient
				.getResultString(cancel_response.getEntity());
		setLogString("response :'" + resultValueString + "'", true);
	...
	public void drcancelBadJson(final String drUrl, final String programID,
			final String targetType, final String targetALLJson,
			final String cancel_url, final String cancel_bad_json,
			final String cancel_dr_json) throws JSONException {
		@SuppressWarnings("unused")
		long timeStamp = System.currentTimeMillis();
		String createUrl = drUrl;
		...
		final HttpResponse cancel_response = HTTPSClient.putResponse(cancelURL,
				cancel_json, HTTPSClient.getPKCSKeyHttpClient(
						"nopartnercode.p12", "ecofactor"));
		Assert.assertTrue(
				cancel_response.getStatusLine().getStatusCode() == 500,
				"Error Status:" + cancel_response.getStatusLine());
		final String resultValueString = HTTPSClient
				.getResultString(cancel_response.getEntity());
		setLogString("response :'" + resultValueString + "'", true);
		...
		final HttpResponse cancel_response1 = HTTPSClient.putResponse(
				cancelURL, cancel_good_json, HTTPSClient.getPKCSKeyHttpClient(
						"nopartnercode.p12", "ecofactor"));
		Assert.assertTrue(
				cancel_response1.getStatusLine().getStatusCode() == 200,
				"Error Status:" + cancel_response.getStatusLine());
	...
	public String commonJson(final String url, final String targetJson) {
		...
		json = json.replaceFirst(
				...
				Long.toString(DateUtil
						.addToUTCMilliSeconds(Calendar.MINUTE, 10)))
				.replaceFirst(
						"<end_time>",
