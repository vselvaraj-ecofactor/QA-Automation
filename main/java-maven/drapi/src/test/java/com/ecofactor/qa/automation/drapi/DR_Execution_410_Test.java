...
public class DR_Execution_410_Test extends AbstractTest {
	...
	public void drEventForEco(final String drUrl, final String programID,
		...
		json = json
				.replaceFirst(
						...
						Long.toString(DateUtil.addToUTCMilliSeconds(
								Calendar.MINUTE, 5))).replaceFirst(
						"<end_time>",
						Long.toString(DateUtil.addToUTCMilliSeconds(
								Calendar.MINUTE, 25)));
		...
		final List<String> locIdStatus = lspEventLocation
				.fullListByLocationIdStatus(proramEventId);
		setLogString("Location and Status after few seconds : " + locIdStatus,
				true);
		...
		final Map<String, Object> detail = lspEventReport
				.updatedDetails(proramEventId);
	 ...
	 *             the parse exception
	 */
	@Test(groups = { Groups.SANITY1 }, dataProvider = "createDRAllGatewaysNVEExecution", dataProviderClass = DRAPIDataProvider.class, priority = 2)
	public void drEventForNve(final String drUrl, final String programID,
			final String eventID, final String targetType,
			final String targetAllJson, final String tstId)
			throws ParseException {
		...
		final HttpResponse response = HTTPSClient.postResponse(directURL, json,
				HTTPSClient.getPKCSKeyHttpClient("ecofactorqanve.p12",
						"ecofactor"));
		Assert.assertTrue(response.getStatusLine().getStatusCode() == 200,
				"Error Status:" + response.getStatusLine());

		final String resultValueString = HTTPSClient.getResultString(response
				.getEntity());
	 ...
	 */
	public List<Thermostat> getCountOfThermostatBasedStatus(
			final Integer programEventId,
			final List<EcpCoreLSEventLocation> locationByStatus,
			final String status) {
		...
		if (status.equalsIgnoreCase(ASTATUS)) {
			...
			for (final EcpCoreLSEventLocation locationEvent : locationByStatus) {
				...
				list1.addAll(tsat.countOfThermostatByStatus(
						locationEvent.getLocationid(), ASTATUS));
			}
	 ...
	 */
	public Map<String, Integer> getCountOfTstBasedOnLocations(
			final Integer proramEventId) {
		...
		// count of tst based on active
		tstByActiveStatus = getCountOfThermostatBasedStatus(proramEventId,
				locationByStatus, ASTATUS);
		locationCount.put("Total No of Thermostats based on Active Status",
				tstByActiveStatus.size());
		...
		// count of thst by skipped
		tstBySkippedStatus = getCountOfThermostatBasedStatus(proramEventId,
				locationByStatus1, SSTATUS);
		locationCount.put("Total No of Thermostats based on Skipped Status",
				tstBySkippedStatus.size());
