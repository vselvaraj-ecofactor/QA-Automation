 ...
 * @version $Rev: 33622 $ $Date: 2015-02-20 10:59:52 +0530 (Fri, 20 Feb 2015) $
 */
...
public class UserManagementTest {
	...
	/** The user management. */
	@Inject
	private InsiteInstallationPage installationPage;
	...
	public void initMethod(Object[] param, Method method) {
		...
		try {
			...
			if (method.getName() == "testResetPswd"
	...
	public void testCreateNewUser(final String userName, final String password,
			...
			final String emailUrl, final String emailUserName,
			final String emailPassword, final String subject,
			final String emailAddress, final String accountUserName,
			final String activeUser, final String partnerType,
		...
		userManagement.logout();
		String temporaryPassword = gmail.getChangedPassword(GMAIL_URL,
				emailUserName, G_PASSWORD, USER_SUBJECT, 0, 0);
	...
	public void clickResetForCreateNewUser(final String userName,
		...
		userManagement.checkForUserReset(DriverConfig.getDriver(), firstName,
				lastName, emailAddress, accountUserName, activeUser,
				partnerType, partner, streetAddress1, streetAddress2, city,
				state, zip, country, mobilePhoneNumber, homePhoneNumber, fax,
				availableRole);
	}
	 ...
	 */
	@Test(dataProvider = "resetNewUser", dataProviderClass = UserDataProvider.class, groups = { "sanity" })
	public void clickCancelForCreateNewUser(final String userName,
			final String password, final String firstName,
			final String lastName, final String emailUrl,
			final String emailUserName, final String emailPassword,
			final String subject, final String emailAddress,
		...
		userManagement.checkForUserCancel(DriverConfig.getDriver(), firstName,
				lastName, emailAddress, accountUserName, activeUser,
				partnerType, partner, streetAddress1, streetAddress2, city,
				state, zip, country, mobilePhoneNumber, homePhoneNumber, fax,
				availableRole);
	 ...
	 * @param searchName
	 *            the search name
	 * @param resetPswdUserName
	 *            the reset pswd user name
	 * @param emailUrl
	 *            the email url
	 * @param emailUserName
	 *            the email user name
	...
	public void testResetPswd(final String userName, final String password,
			final String searchName, final String resetPswdUserName,
			final String emailUrl, final String emailUserName,
			final String emailPassword, final String subject,
		...
		userManagement.checkResetPswd(GMAIL_URL, emailUserName, G_PASSWORD,
				RESETPASSWORD_SUBJECT, searchName, resetPswdUserName,
				newPassword);
	}
	...
	public void verifyAccountDetails(final String userName,
	 ...
	 *            the search email id
	 */
	@Test(dataProvider = "searchEmilEmailId", dataProviderClass = UserDataProvider.class, retryAnalyzer = RerunFailTestAnalyzer.class, groups = { "ui" })
	 ...
	 *            the search email id
	 */
	@Test(dataProvider = "searchEmilAdminEmailId", dataProviderClass = UserDataProvider.class, retryAnalyzer = RerunFailTestAnalyzer.class, groups = { "ui" })
	public void verifySaveLink(final String userName, final String password,
			final String searchEmailId, final String firstName,
			final String lastName, final String emailId,
			final String activeUser, final String streetAddress1,
			final String streetAddress2, final String city, final String state,
			final String zip, final String country,
			final String mobilePhoneNum, final String pnoneNumber,
			final String faxNumber) {

		userManagement.searchByEmail(searchEmailId);
		tinyWait();
		userManagement.clickTopFirstUser();
		Map<String, String> assingedRoleDetail = userManagement
				.getRoleDetails();
		userManagement.clickEdit();
		Map<String, String> accountDetailMap = userManagement
				.getAccountDetails();
		userManagement.clickContactDetailsTab();
		Map<String, String> accountContactMap = userManagement
				.getContactDetails();
		userManagement.clickAccountDetailsTab();
		userManagement.editFirstName(firstName);
		userManagement.clickSave();
		userManagement.searchByEmail(searchEmailId);
		tinyWait();
		userManagement.clickTopFirstUser();
		userManagement.clickEdit();
		userManagement.changeAccountDetails(firstName, lastName, emailId,
				activeUser);
		userManagement.clickContactDetailsTab();
		userManagement.changeContactDetails(streetAddress1, streetAddress2,
				city, state, zip, country, mobilePhoneNum, pnoneNumber,
				faxNumber);
		userManagement.checkAssignedRoleDetails(assingedRoleDetail);
		userManagement.clickAccountDetailsTab();
		userManagement.changeAccountDetails(
				accountDetailMap.get(userConfig.get(FIRST_NAME)),
				accountDetailMap.get(userConfig.get(LAST_NAME)),
				accountDetailMap.get(userConfig.get(EMAIL_FIELD)),
				accountDetailMap.get(userConfig.get(USER_STATUS)));
		userManagement.clickContactDetailsTab();
		userManagement.changeContactDetails(
				accountContactMap.get(userConfig.get(STREET_ADDRESS1)),
				accountContactMap.get(userConfig.get(STREET_ADDRESS2)),
				accountContactMap.get(userConfig.get(CITY)),
				accountContactMap.get(userConfig.get(STATE)),
				accountContactMap.get(userConfig.get(ZIP)),
				accountContactMap.get(userConfig.get(COUNTRY)),
				accountContactMap.get(userConfig.get(MOBILE_NUM)),
				accountContactMap.get(userConfig.get(PHONE_NUM)),
				accountContactMap.get(userConfig.get(FAX_NUM)));
		userManagement.clickSave();
	}
	...
	public void editAndSave(final String userName, final String password,
			final String searchEmail, final String firstName,
		...
		Map<String, String> accountDetailMap = userManagement
				.getAccountDetails();
		userManagement.changeAccountDetails(firstName, lastName, emailAddress,
				activeUser);
		userManagement.clickSave();
		userManagement.searchByEmail(searchEmail);
		tinyWait();
		...
		userManagement.clickEdit();
		userManagement.changeAccountDetails(
				accountDetailMap.get(FIRST_NAME_KEY),
				accountDetailMap.get(LAST_NAME_KEY),
				accountDetailMap.get(EMAIL_KEY),
	 ...
	 *            the nve password2
	 */
	@Test(dataProvider = "loginToNVEUsers", dataProviderClass = UserDataProvider.class, groups = { "sanity" })
	public void verifyNVEUser(final String nveUserName2,
			final String nvePassword2, final String nveUserName1,
			final String nvePassword1) {

		Assert.assertFalse(userManagement.getMenu().size() > 1,
				"Another Menu Available");
		Assert.assertTrue(
				userManagement.isMenuDisplayed(insiteConfig.get(ON_BOARDING)),
				"SubMenu Not Displayed");
		userManagement.logout();
		userManagement.loadPage(nveUserName1, nvePassword1);
		Assert.assertFalse(userManagement.getMenu().size() > 1,
				"Another Menu Available");
		Assert.assertTrue(
				userManagement.isMenuDisplayed(insiteConfig.get(INSTALLATION)),
				"SubMenu Not Displayed");
	}
	 ...
	 * @param password
	 *            the password
	 * @param accountSearchName
	 *            the account search name
	 * @param accountUserName
	 *            the account user name
	 */
	@Test(dataProvider = "assignRoleToUser", dataProviderClass = UserDataProvider.class, groups = { "sanity" }, priority = 2)
	public void testAssignRoleToExistingUser(final String userName,
			final String password, final String accountSearchName,
			final String accountUserName) {

		userManagement.searchGivenUser(accountSearchName);
		userManagement.verifyUserAndClickEditButton(accountSearchName);
		String assignedRole = userManagement
				.assignRoleToUser(accountSearchName);
	 ...
	 *            the password
	 */
	@Test(dataProvider = "loginToEmils", dataProviderClass = UserDataProvider.class, retryAnalyzer = RerunFailTestAnalyzer.class, groups = { "smoke" })
	public void installationLink(final String userId, final String password) {

		installationPage.clickInstallationTab();
		Assert.assertTrue(installationPage.isInstallationTabDisplayed(),
				"Error in displaying installation tab");
	}

	/**
	 * IP-40. Installation schedule link.
	 * 
	 * @param userId
	 *            the user id
	 * @param password
	 ...
	 *            the street address
	 */
	@Test(dataProvider = "loginToScheduleLink", dataProviderClass = UserDataProvider.class, retryAnalyzer = RerunFailTestAnalyzer.class, groups = { "smoke" })
	public void installationScheduleLink(final String userId,
		...
		Assert.assertTrue(installationPage.isInstallationTabDisplayed(),
				"Error in displaying installation tab");
		installationPage.clickSchedulingPage();
		Assert.assertTrue(installationPage.isSchedulingPageDisplayed(),
				"Error in displaying scheduling tab");
		installationPage.searchByStreetAddress(streetAddress);
		installationPage.clickFind();
		Assert.assertTrue(installationPage
				.isSearchedUserDisplayed(streetAddress));
	}

	/**
	 * IP-41. Installation pre config link.
	 * 
	 * @param userId
	 *            the user id
	 * @param password
	 ...
	 *            the street address
	 */
	@Test(dataProvider = "loginToPreconfigLink", dataProviderClass = UserDataProvider.class, retryAnalyzer = RerunFailTestAnalyzer.class, groups = { "smoke" })
	public void installationPreConfigLink(final String userId,
			final String password, final String streetAddress) {

		installationPage.clickInstallationTab();
		Assert.assertTrue(installationPage.isInstallationTabDisplayed(),
				"Installation page is not displayed");
		installationPage.clickPreConfigTab();
		Assert.assertTrue(installationPage.isPreConfigTabDisplayed(),
				"Preconfiguration page is not displayed");
		installationPage.searchByStreetAddress(streetAddress);
		installationPage.clickFind();
		installationPage.clickTopFirstUser();
		Assert.assertTrue(installationPage.isTstNamePageDisplayed(),
				"Thermostat name page has not been displayed");
	}

	/**
	 * IP-42. User management link.
	 * 
	 * @param userId
	 *            the user id
	 * @param password
	...
	public void userManagementLink(final String userId, final String password) {
		...
		installationPage.selectUserManagementPage();
		Assert.assertTrue(installationPage.isUserManagementPageDisplayed(),
				"User Management page has not been displayed");
	}

	/**
	 * IP-43. Role management link.
	 * 
	 * @param userId
	 *            the user id
	 * @param password
	 *            the password
	 */
	@Test(dataProvider = "loginToEmilAdmin", dataProviderClass = UserDataProvider.class, retryAnalyzer = RerunFailTestAnalyzer.class, groups = { "smoke" })
	public void roleManagementLink(final String userId, final String password) {

		installationPage.selectRoleManagementPage();
		Assert.assertTrue(installationPage.isRoleManagementPageDisplayed(),
				"Role Management page has not been displayed");
	}
	...
	public void findValidEmailAddress(final String userName,
			final String password, final String emailAddress) {

		userManagement.searchByEmail(emailAddress);
		Assert.assertTrue(
				userManagement.isEmailDisplayedInSearchGrid(emailAddress),
	...
	public void findPartialEmailAddress(final String userName,
			final String password, final String emailAddress) {
	...
	public void findValidUserName(final String userName, final String password,
		...
		Assert.assertTrue(
				userManagement.isUserNameDisplayedInSearchGrid(findUserName),
	...
	public void findPartialUserName(final String userName,
			final String password, final String findUserName) {
	...
	public void sortNamesInSearchList(final String userName,
			final String password) {

		userManagement.clickNameLabel();
		Assert.assertTrue(userManagement.isSortedAscending(),
				"Names Not Sorted in Ascending");
		userManagement.clickNameLabel();
		Assert.assertTrue(userManagement.isSortedDescending(),
				"Names Not Sorted in Descending");
	}
	...
	public void testFirstLink(String userName, String password) {
		...
		Assert.assertTrue(userManagement.getCurrentPageNo() == 1,
				"Pagination differs");
	}
	...
	public void testPreviousLink(String userName, String password) {
		...
		Assert.assertTrue(userManagement.getCurrentPageNo() == 2,
				"Pagination differs");
	...
	public void testLastLink(String userName, String password) {
		...
		Assert.assertTrue(userManagement.getCurrentPageNo() == userManagement
				.getLastPageNo(), "Pagination differs");
	}
}
