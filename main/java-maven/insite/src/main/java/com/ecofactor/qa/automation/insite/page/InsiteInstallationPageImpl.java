 ...
 */
public class InsiteInstallationPageImpl extends InsiteAuthenticatedPageImpl
		implements InsiteInstallationPage {
	...
	private static Logger logger = LoggerFactory
			.getLogger(InsiteAuthenticatedPageImpl.class);
	...
	public void searchByStreetAddress(final String streetAddress) {
		...
		DriverConfig.setLogString("Find Street Address : " + streetAddress,
				true);
	...
	public void clickPreConfigTab() {
		...
		final WebElement subMenu = DriverConfig.getDriver().findElement(
				By.id(SUBMENU));
		WebElement rolesLink = retrieveSubElementByTagText(
				DriverConfig.getDriver(), subMenu, TAG_ANCHOR,
	...
	public boolean isTstNamePageDisplayed() {
		...
		DriverConfig.setLogString("Verify Thermostat Name Page Displayed.",
				true);
		mediumWait();
		return isDisplayedByClassName(DriverConfig.getDriver(), TSTNAME,
				SHORT_TIMEOUT);
	}
	...
	public boolean isPreConfigTabDisplayed() {
		...
		DriverConfig.setLogString("Verify PreConfiguration Page Displayed.",
				true);
		mediumWait();
		final WebElement subMenu = DriverConfig.getDriver().findElement(
				By.id(SUBMENU));
	...
	public boolean isSearchedUserDisplayed(final String streetAddress) {
		...
		return isDisplayedByXpath(DriverConfig.getDriver(), USER_DETAILS,
				MEDIUM_TIMEOUT);
	}
	...
	public boolean isUserManagementPageDisplayed() {
		...
		DriverConfig.setLogString("verify User Management Page displayed.",
				true);
		final WebElement subElement = DriverConfig.getDriver().findElement(
				By.id(insiteConfig.get(SUB_MENU)));
		WebElement rolesLink = retrieveSubElementByTagText(
				DriverConfig.getDriver(), subElement, TAG_ANCHOR,
				insiteConfig.get(USER_MNGMNT), SHORT_TIMEOUT);
		return rolesLink.isDisplayed();
	}
	...
	public boolean isRoleManagementPageDisplayed() {
		...
		DriverConfig
				.setLogString("verify RoleManagement Page displayed.", true);
		final WebElement subElement = DriverConfig.getDriver().findElement(
				By.id(insiteConfig.get(SUB_MENU)));
		WebElement rolesLink = retrieveSubElementByTagText(
				DriverConfig.getDriver(), subElement, TAG_ANCHOR,
	...
	private void selectPage(String pageName) {
		...
		if (menuItems != null && menuItems.size() > 0) {
			...
			List<WebElement> linkElement = linkElements.findElements(By
					.tagName(TAG_ANCHOR));
			if (linkElement != null && linkElement.size() > 0) {
				for (WebElement element : linkElement) {
					smallWait();
					if (element.getText().equalsIgnoreCase(
							insiteConfig.get(pageName))) {
